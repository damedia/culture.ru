var armdMuseumGuide = {
    ...
    initPdf : function (url) {
        ...
        zoomIn: function (scale) {
            this.scale = Math.min( 4.0, Math.ceil((null == scale ? this.scale + 0.20 : scale).toFixed(2) * 10) / 10);
            this.disableButtons();
            this.updateDimensions();
            return this.render();
        },
        zoomOut: function (scale) {
            this.scale = Math.max( 0.25, Math.ceil((null == scale ? this.scale - 0.20 : scale).toFixed(2) * 10) / 10);
            this.disableButtons();
            this.updateDimensions();
            return this.render();
        },
        fullscreen: function pdfViewFullscreen() {
            ...
            this.zoomIn(1.5);
            this.previousDims = {
                viewerW : $('#pdf-viewer').css('width'),
                viewerH : $('#pdf-viewer').css('height'),
                canvasW : $('#pdf-canvas-container').css('max-width'),
                canvasH : $('#pdf-canvas-container').css('height')
            };
            //$('#pdf-viewer').css('width', '100%');
            $('#pdf-viewer').css('height', '100%');
            $('#pdf-canvas-container').css('max-width', '90%');
            //$('#pdf-canvas-container').css('height', '100%');
            return true;
        },
        fullscreenExit: function() {
            ...
            if (isFullscreen) {
                ...
                //$('#pdf-viewer').css('width', pdfViewer.previousDims.viewerW);
                $('#pdf-viewer').css('height', pdfViewer.previousDims.viewerH);
                $('#pdf-canvas-container').css('max-width', pdfViewer.previousDims.canvasW);
                //$('#pdf-canvas-container').css('height', pdfViewer.previousDims.canvasH);
    ...
    PDFJS.getDocument(url).then(function getPdfHelloWorld (_pdfDoc) {
        pdfDoc = _pdfDoc;
        pdfViewer.init();
        $(document).on('webkitfullscreenchange mozfullscreenchange fullscreenchange', pdfViewer.fullscreenExit);
    });
}
