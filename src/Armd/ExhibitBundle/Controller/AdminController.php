...
class AdminController extends Controller
{
    ...
    public function testAction()
    {
        ...
        switch($testType) {
            case 'list_without_3d':
                $objects = $this->getDoctrine()->getManager()->getRepository('ArmdExhibitBundle:Object')
    ...
    /**
     * @Route("/category_up/{id}", name="armd_exhibit_admin_category_tree_up")
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     */
    public function upAction($id)
    {
        $em = $this->getDoctrine()->getManager();
        $repo = $em->getRepository('ArmdExhibitBundle:Category');
    ...
    /**
     * @Route("/category_down/{id}", name="armd_exhibit_admin_category_tree_down")
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     */
    ...
    /**
     * @Route("/delete_category/{id}", name="armd_exhibit_admin_category_delete")
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     */
    ...
    {
        $exportDir = realpath(__DIR__ . '/../../../../tmp/art_objects');

        $objects = $this->getDoctrine()->getManager()
            ->getRepository('ArmdExhibitBundle:Object')
            ->createQueryBuilder('o')
        ...
        $categories = $this->getDoctrine()->getManager()->getRepository('ArmdExhibitBundle:Category')->childrenHierarchy();

        $content = $this->renderView('ArmdExhibitBundle:Admin:export_objects.xml.twig', array(
            'objects' => $objects,
        ...
        if ($format === 'xml') {
            ...
            foreach($objects as $object) {
                $filePath = $exportDir . '/art_object_' . $object->getId() . '.txt';
                $content = $this->renderView('ArmdExhibitBundle:Admin:export_objects.txt.twig', array('object' => $object));
    ...
    {
        $dir = realpath(__DIR__ . '/../../../../tmp/art_objects_trans_import');
        if (!is_dir($dir)) {
        ...
        while (($file = readdir($dh)) !== false) {
            ...
            $object = $em->getRepository('ArmdExhibitBundle:ArtObject')->find($matches[0]);
    ...
    public function rebuildTreeAction()
    {
        ...
        $repairer->rebuildTree($em, $em->getRepository('ArmdExhibitBundle:Category'));
        return new Response('ok');
    }
    ...
    public function verifyTreeAction()
    {
        $errors = $this->getDoctrine()->getManager()
            ->getRepository('ArmdExhibitBundle:Category')->verify();
        var_dump($errors);
        return new Response();
    }
     ...
     * name="armd_exhibit_admin_list_user_objects",
     * options={"expose"=true})
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     * @Template("ArmdExhibitBundle:Admin:list_user_objects.html.twig")
     */
    public function listUserObjectsAction($userId)
    {
        ...
        $userObjects = $this->get('armd_exhibit.manager.object')->getUserObjects($user);

        $qbOtherObjects = $em->getRepository('ArmdExhibitBundle:ArtObject')
            ->createQueryBuilder('o')
            ->orderBy('o.title', 'ASC');
     ...
     *  name="armd_exhibit_admin_grant_user_object",
     *  options={"expose"=true}
     * )
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     */
    public function grantUserObjectAction($userId, $objectId)
    {
        ...
        $object = $em->getRepository('ArmdExhibitBundle:ArtObject')->find($objectId);
        if(empty($object)) {
            throw new \InvalidArgumentException('Exhibit object not found');
        }
...
//        return new Response(json_encode(array('resultCode' => 'OK')));
        return $this->forward('ArmdExhibitBundle:Admin:listUserObjects', array('userId' => $userId));
    }

    /**
     * @Route("/revoke-user-object/user/{userId}/object/{objectId}",
     *  name="armd_exhibit_admin_revoke_user_object",
     *  options={"expose"=true}
     * )
     * @Secure(roles="ROLE_SUPER_ADMIN,ROLE_SONATA_ADMIN")
     */
