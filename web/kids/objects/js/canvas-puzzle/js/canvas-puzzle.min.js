(function () {
    ...
    Util.calcPieces = function (img, tmpl) {
        ...
        // Custom Select
        var optLength = jQuery("#set-parts option").length,
            lineNum = 0,
            optLines = [];
        
        jQuery("#set-parts option").each(function(){
            var active = (jQuery(this).index() == 0) ? 'active' : '';
            optLines.push('<li  class="'+active+'"><a href="#'+lineNum+'" data-value="'+jQuery(this).val()+'">'+jQuery(this).html()+'</a></li>');
            lineNum++;
        })
        jQuery('#select-options').html(optLines.join(''));   
    ...
    jigsaw.Jigsaw = Class.extend({
        init: function (opts) {
            ...
            function resizeView() {
                ...
                cv1.width = document.documentElement.clientWidth + (isMyStuffScrollingY() ? 100 : 0) , 
                cv1.height = document.documentElement.clientHeight - 45  + (isMyStuffScrollingX() ? 100 : 0) ;
                self.ce.redraw();
            ...
            function isMyStuffScrollingY() {
                var docHeight = jQuery(document).height();
                var winHeight = jQuery(window).height();
                return (docHeight > winHeight);
            }
            
            function isMyStuffScrollingX() {
        ...
        render: function () {
            ...
            if ( jQuery.browser.msie && jQuery.browser.version <= 8.0) {
                var jqImageSrc = jQuery('.puzzle').attr('src');
                var jqImageWidth = jQuery('.puzzle').width();
                var jqImageHeight = jQuery('.puzzle').height();
            jQuery('#image-preview').html('<img  src="'+jqImageSrc+'" alt="" />').css({'width':size * cols, 'height':size * rows});
            } else {
...
(function () {
    ...
    jigsaw.UI = Class.extend({
        ...
        show_preview: function () {
            ...
            if (canvas.className == "show" )
                jQuery('#SHOW_PREVIEW').addClass('active');
            else 
                jQuery('#SHOW_PREVIEW').removeClass('active');
        ...
        show_help: function () {
            ...
            jQuery('#SHOW_HELP').addClass('active');
        }
    });
    ...
    function change_select(self, eventBus) {
       jQuery('#select-options').on(Cevent.isTouchDevice ? "touchstart" : "click", 'a', function(){
            var chosen = jQuery(this).attr('data-value');

            jQuery(this).parent().addClass('active').siblings().removeClass('active');
            
            jQuery('#set-parts option[value='+chosen+']').attr('selected', 'selected');
           
            eventBus.emit(jigsaw.Events.PARTS_NUMBER_CHANGED, +chosen);
            eventBus.emit(jigsaw.Events.RENDER_REQUEST)
            
            jQuery('#chosen-option').html(jQuery(this).html());
...
(function (document, window, undefined) {
    ...
    function closeModal(e) {
        ...
        jQuery('#game-options a.active').removeClass('active');
